...
main() {
  SearchArtistFilterBloc bloc;

  setUp(() {
    bloc = SearchArtistFilterBloc();
  });

  test('should update artist type', () async {
    bloc.updateArtistType('Circle');

    await expectLater(bloc.artistType$, emits('Circle'));

    expect(bloc.artistType, equals('Circle'));
  });
  ...
  test('should return params correctly', () {
    ...
    expect(params.containsKey('artistTypes'), isFalse);
    ...
    expect(params.containsKey('artistTypes'), isTrue);
    expect(params['artistTypes'], equals('Circle'));
